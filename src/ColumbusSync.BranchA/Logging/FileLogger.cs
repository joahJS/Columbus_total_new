...
namespace ColumbusSync.BranchA.Logging
{
    /// <summary>
    /// 이 프로그램은 화면 없이(콘솔조차 안 보고) 백그라운드로 오래 돌아가는 것을 전제로 하므로,
    /// 콘솔 출력과 별개로 파일에도 항상 로그를 남긴다. 문제가 생겼을 때 담당자가 나중에
    /// 로그 파일만 열어봐도 원인을 추적할 수 있어야 한다.
    /// </summary>
