...
{
    /// <summary>
    /// 통합 허브 DB(COLUMBUS_WEIGH_HUB)의 dbo.CUSTOMER 테이블을 조회하는 실제 구현체.
    /// 이 프로그램은 조회 전용이므로 INSERT/UPDATE/DELETE는 하지 않는다(거래처 등록/수정은
    /// 각 지점이 지금 쓰는 프로그램에서 그대로 처리하고, 동기화 잡이 이 테이블을 채운다).
    /// </summary>
    public sealed class SqlCustomerRepository : ICustomerRepository
    {
        private const string SelectSql = @"
SELECT CUSTOMER_ID, BRANCH_CODE, SOURCE_CODE, CUSTOMER_NAME, CEO_NAME, MANAGER_NAME, TEL, FAX, REMARK, SYNCED_AT
FROM dbo.CUSTOMER
WHERE (@Search = '' OR CUSTOMER_NAME LIKE '%' + @Search + '%' OR SOURCE_CODE LIKE '%' + @Search + '%')
ORDER BY BRANCH_CODE, SOURCE_CODE";

        public BindingList<CustomerRecord> Records { get; } = new BindingList<CustomerRecord>();
        ...
        private static CustomerRecord ToRecord(DataRow row)
        {
            return new CustomerRecord
            {
                Id = Convert.ToInt32(row["CUSTOMER_ID"]),
                BranchCode = AsString(row, "BRANCH_CODE"),
                CustomerCode = AsString(row, "SOURCE_CODE"),
